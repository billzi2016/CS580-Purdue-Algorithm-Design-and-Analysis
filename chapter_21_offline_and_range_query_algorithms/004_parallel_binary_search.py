...
def parallel_binary_search(
    events: list[int], thresholds: list[int]
) -> list[int | None]:
    """返回每个阈值首次被事件前缀和达到的时间；真正按中点分桶扫描。"""
    if any(value < 0 for value in events):
        raise ValueError("事件增量必须非负以保证单调性")
    total = sum(events)
    low = [0] * len(thresholds)
    high = [len(events)] * len(thresholds)
    for i, value in enumerate(thresholds):
        if value <= 0:
            ...
            low[i] = len(events) + 1
    while True:
        buckets = [[] for _ in range(len(events) + 1)]
        pending = False
        for i in range(len(thresholds)):
            if low[i] < high[i]:
                middle = (low[i] + high[i]) // 2
        ...
        for time in range(len(events) + 1):
            if time:
                prefix += events[time - 1]
            for i in buckets[time]:
                if prefix >= thresholds[i]:
                    high[i] = time
                else:
                    low[i] = time + 1

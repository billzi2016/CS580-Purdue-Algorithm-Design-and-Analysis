...
def maximum_bipartite_matching(graph: dict[str, list[str]]) -> dict[str, str]:
    ...
    def augment(left: str, seen: set[str]) -> bool:
        for right in graph[left]:
            ...
            seen.add(right)
            if right not in matched or augment(matched[right], seen): matched[right] = left; return True
        return False
    for left in graph: augment(left, set())
    return matched

if __name__ == "__main__":
    matching = maximum_bipartite_matching({"a": ["1", "2"], "b": ["1"], "c": ["2", "3"]})

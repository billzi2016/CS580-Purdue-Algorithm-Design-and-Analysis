...
class SparseTable:
    ...
    def __init__(self, values: list[int]) -> None:
        ...
        self.table = [values[:]] if values else []
        width = 1
        while 2 * width <= self.length:
            ...
            self.table.append(
                [
                    ...
                    for i in range(self.length - 2 * width + 1)
                ]
            )
            width *= 2
    ...
    def range_min(self, left: int, right: int) -> int:
        ...
        width = 1 << level
        return min(self.table[level][left], self.table[level][right - width + 1])


if __name__ == "__main__":
    table = SparseTable([4, 6, 1, 5, 7, 3])
    assert table.range_min(1, 4) == 1 and table.range_min(3, 5) == 3
    print("007_sparse_table: all examples passed")

...
class LazySegmentTree:
    ...
    def __init__(self, values: list[int]) -> None:
        ...
        self.sum = [0] * max(1, 4 * self.length)
        self.lazy = [0] * max(1, 4 * self.length)
        if values:
            self._build(1, 0, self.length, values)
    ...
    def range_sum(self, query_left: int, query_right: int) -> int:
        ...
        def visit(node: int, left: int, right: int) -> int:
            ...
            self._push(node, left, right)
            middle = (left + right) // 2
            return visit(2 * node, left, middle) + visit(2 * node + 1, middle, right)
...
if __name__ == "__main__":
    ...
    assert tree.range_sum(0, 4) == 16 and tree.range_sum(1, 3) == 9
    tree.range_add(0, 0, 5)
    assert LazySegmentTree([]).range_sum(0, 0) == 0
    print("006_lazy_segment_tree: all examples passed")

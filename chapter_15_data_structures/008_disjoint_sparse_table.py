...
class DisjointSparseTable:
    ...
    def __init__(self, values: list[int]) -> None:
        ...
        while block // 2 < self.length:
            ...
            for start in range(0, self.length, block):
                middle = min(start + half, self.length)
                end = min(start + block, self.length)
                if middle > start:
                    row[middle - 1] = values[middle - 1]
                    for index in range(middle - 2, start - 1, -1):
                        row[index] = values[index] + row[index + 1]
                if middle < end:
                    row[middle] = values[middle]
                    for index in range(middle + 1, end):
                        row[index] = row[index - 1] + values[index]
    ...
    def range_sum(self, left: int, right: int) -> int:
        ...
        return self.table[level][left] + self.table[level][right]


if __name__ == "__main__":
    table = DisjointSparseTable([1, 2, 3, 4, 5])
    assert (
        table.range_sum(1, 3) == 9
        and table.range_sum(0, 4) == 15
        and table.range_sum(2, 2) == 3
    )
    print("008_disjoint_sparse_table: all examples passed")

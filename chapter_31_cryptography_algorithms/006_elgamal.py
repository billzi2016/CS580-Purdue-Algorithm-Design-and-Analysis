...
def elgamal_public_value(prime_modulus: int, generator: int, private_key: int) -> int:
    ...
    if any(isinstance(value, bool) or not isinstance(value, int) for value in (prime_modulus, generator, private_key)) or prime_modulus <= 3 or not 2 <= generator < prime_modulus or not 1 <= private_key <= prime_modulus - 2:
        raise ValueError("ElGamal 参数范围无效")
    return _power(generator, private_key, prime_modulus)


def elgamal_encrypt(message: int, prime_modulus: int, generator: int, public_value: int, ephemeral_exponent: int) -> tuple[int, int]:
    """返回 (c1,c2)=(g^k, m*y^k) mod p；k 必须每次新鲜且随机，本函数不生成它。"""
    if any(isinstance(value, bool) or not isinstance(value, int) for value in (message, prime_modulus, generator, public_value, ephemeral_exponent)) or not 0 <= message < prime_modulus or not 2 <= generator < prime_modulus or not 2 <= public_value < prime_modulus or not 1 <= ephemeral_exponent <= prime_modulus - 2:
        raise ValueError("ElGamal 输入范围无效")
    first = _power(generator, ephemeral_exponent, prime_modulus)
    return first, message * _power(public_value, ephemeral_exponent, prime_modulus) % prime_modulus


def elgamal_decrypt(ciphertext: tuple[int, int], prime_modulus: int, private_key: int) -> int:
    ...
    if any(isinstance(value, bool) or not isinstance(value, int) for value in (first, second, prime_modulus, private_key)) or prime_modulus <= 3 or not 1 <= first < prime_modulus or not 0 <= second < prime_modulus or not 1 <= private_key <= prime_modulus - 2:
        raise ValueError("ElGamal 输入范围无效")
    return second * _inverse(_power(first, private_key, prime_modulus), prime_modulus) % prime_modulus

...
def differential_evolution(objective: Callable[[tuple[float, ...]], float], bounds: list[tuple[float, float]], population_size: int = 20, generations: int = 100, differential_weight: float = 0.8, crossover_rate: float = 0.7, seed: int | None = None) -> DEResult:
    """以 DE/rand/1/bin 在盒约束内最大化目标。

    关键点：每个目标向量只和自己的试验向量比较，贪心选择保证该位置适应度不会下降。
    """
    if not bounds or population_size < 4 or generations < 0 or any(low > high for low, high in bounds) or not 0 <= crossover_rate <= 1:
        raise ValueError("DE 参数或边界无效")
    rng = random.Random(seed)
    population = [tuple(rng.uniform(low, high) for low, high in bounds) for _ in range(population_size)]
    ...
    for _ in range(generations):
        for target_index, target in enumerate(population):
            ...
            for dimension, (low, high) in enumerate(bounds):
                mutant = population[first][dimension] + differential_weight * (population[second][dimension] - population[third][dimension])
                value = mutant if dimension == forced_dimension or rng.random() < crossover_rate else target[dimension]
            ...
            if candidate_score >= scores[target_index]:
                population[target_index], scores[target_index] = candidate, candidate_score
...
if __name__ == "__main__":
    result = differential_evolution(lambda point: -((point[0] - 1.5) ** 2), [(-4, 4)], population_size=20, generations=100, seed=6)

...
def particle_swarm_optimize(objective: Callable[[tuple[float, ...]], float], bounds: list[tuple[float, float]], particles: int = 20, iterations: int = 100, inertia: float = 0.7, cognitive: float = 1.4, social: float = 1.4, seed: int | None = None) -> PSOResult:
    """在盒约束内最大化连续目标。

    参数：每维 bounds 为 ``(low, high)``。返回历史全局最佳。关键点：位置更新后立即评分并更新个人/全局最佳。
    边界：维度为空、边界无效或粒子数非正都会报错。
    """
    if not bounds or particles < 1 or iterations < 0 or any(low > high for low, high in bounds):
        raise ValueError("维度、边界、粒子数或迭代次数无效")
    rng = random.Random(seed)
    positions = [tuple(rng.uniform(low, high) for low, high in bounds) for _ in range(particles)]
    ...
    for _ in range(iterations):
        for index, position in enumerate(positions):
            ...
            for dimension, (low, high) in enumerate(bounds):
                velocity = (inertia * velocities[index][dimension] + cognitive * rng.random() * (personal_best[index][dimension] - position[dimension]) + social * rng.random() * (global_best[dimension] - position[dimension]))
                updated_velocity.append(velocity)
                updated_position.append(min(high, max(low, position[dimension] + velocity)))
...
if __name__ == "__main__":
    result = particle_swarm_optimize(lambda point: -((point[0] - 2) ** 2 + (point[1] + 1) ** 2), [(-5, 5), (-5, 5)], particles=30, iterations=150, seed=4)

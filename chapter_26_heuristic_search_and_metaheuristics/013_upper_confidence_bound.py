...
def ucb1_select(total_rewards: list[float], visits: list[int], exploration: float = math.sqrt(2)) -> int:
    """按 UCB1 选择一个臂；未访问臂保证先被探索。"""
    if not total_rewards or len(total_rewards) != len(visits) or any(count < 0 for count in visits):
        raise ValueError("回报与访问次数无效")
    for index, count in enumerate(visits):
        if count == 0:
            return index
    total = sum(visits)
    return max(range(len(visits)), key=lambda index: total_rewards[index] / visits[index] + exploration * math.sqrt(math.log(total) / visits[index]))

...
def maximum_overlap(intervals:list[tuple[int,int]])->int:
    ...
    for start,end in intervals:
        if start>end:raise ValueError("区间起点不能晚于终点")
        if start!=end:events.append((start,1));events.append((end,-1))
    for i in range(1,len(events)):
        current=events[i];j=i-1
        while j>=0 and events[j]>current:events[j+1]=events[j];j-=1
        events[j+1]=current
    active=best=0
    for _,delta in events:active+=delta;best=max(best,active)
    return best

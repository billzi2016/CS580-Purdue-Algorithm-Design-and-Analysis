...
class LSTMCell(torch.nn.Module):
    ...
    def __init__(self,input_size:int,hidden_size:int)->None:
        """初始化门控线性参数；维度必须为正。"""
        super().__init__()
        if input_size<=0 or hidden_size<=0:raise ValueError("LSTM 维度必须为正")
        self.hidden_size=hidden_size
        self.weight=torch.nn.Parameter(torch.randn(input_size+hidden_size,4*hidden_size)*0.1)
    ...
    def forward(self,inputs:torch.Tensor)->tuple[torch.Tensor,torch.Tensor]:
        ...
        """
        if inputs.ndim!=3 or inputs.shape[2]!=self.weight.shape[0]-self.hidden_size:raise ValueError("inputs 形状不匹配")
        batch,time,_=inputs.shape;hidden=torch.zeros((batch,self.hidden_size),dtype=inputs.dtype);cell=torch.zeros_like(hidden);hidden_states=[];cell_states=[]
        for index in range(time):
            gates=torch.cat((inputs[:,index,:],hidden),dim=1)@self.weight+self.bias
            ...
            hidden_states.append(hidden);cell_states.append(cell)
    ...
    def training_step(self,inputs:torch.Tensor,target:torch.Tensor,learning_rate:float)->float:
        ...
        if hidden.shape!=target.shape:raise ValueError("target 形状不匹配")
        loss=torch.mean((hidden-target)**2);loss.backward()
        with torch.no_grad():
            for parameter in self.parameters():parameter-=learning_rate*parameter.grad
        return float(loss.detach())

if __name__=="__main__":
    cell=LSTMCell(2,3);data=torch.randn((2,4,2));hidden,states=cell(data)
    assert hidden.shape==states.shape==(2,4,3)
    before=cell.weight.detach().clone();assert cell.training_step(data,torch.zeros_like(hidden),0.01)>=0.0

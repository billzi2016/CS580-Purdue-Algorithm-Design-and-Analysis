...
class InceptionBlock(torch.nn.Module):
    ...
    def __init__(self, input_channels:int, channels:tuple[int,int,int,int,int,int]) -> None:
        """创建四分支模块。
        ...
        """
        super().__init__()
        if input_channels<=0 or len(channels)!=6 or any(value<=0 for value in channels): raise ValueError("Inception 通道参数无效")
        one,three_reduce,three,five_reduce,five,pool_proj=channels
        self.branch1=torch.nn.Conv2d(input_channels,one,1)
        self.branch2=torch.nn.Sequential(torch.nn.Conv2d(input_channels,three_reduce,1),torch.nn.ReLU(),torch.nn.Conv2d(three_reduce,three,3,padding=1))
        self.branch3=torch.nn.Sequential(torch.nn.Conv2d(input_channels,five_reduce,1),torch.nn.ReLU(),torch.nn.Conv2d(five_reduce,five,5,padding=2))
        self.branch4=torch.nn.Sequential(torch.nn.MaxPool2d(3,stride=1,padding=1),torch.nn.Conv2d(input_channels,pool_proj,1))
    def forward(self, features:torch.Tensor)->torch.Tensor:
        ...
        """
        if features.ndim!=4: raise ValueError("InceptionBlock 需要 NCHW 输入")
        return torch.cat((self.branch1(features),self.branch2(features),self.branch3(features),self.branch4(features)),dim=1)
...
if __name__=="__main__":
    block=InceptionBlock(3,(2,2,3,2,4,1));data=torch.randn((1,3,8,8));output=block(data)
    assert output.shape==(1,10,8,8)
    before=block.branch1.weight.detach().clone();assert block.training_step(data,torch.zeros_like(output),0.01)>=0.0

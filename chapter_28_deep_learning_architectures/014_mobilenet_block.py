...
class MobileNetBlock(torch.nn.Module):
    ...
    def __init__(self,input_channels:int,output_channels:int,stride:int=1)->None:
        ...
        """
        super().__init__()
        if input_channels<=0 or output_channels<=0 or stride<=0:raise ValueError("MobileNet 参数必须为正")
        self.depthwise=torch.nn.Conv2d(input_channels,input_channels,3,stride=stride,padding=1,groups=input_channels,bias=False)
    ...
    def forward(self,features:torch.Tensor)->torch.Tensor:
        ...
        """
        if features.ndim!=4:raise ValueError("MobileNetBlock 需要 NCHW 输入")
...
if __name__=="__main__":
    block=MobileNetBlock(3,6,stride=2);data=torch.randn((2,3,8,8));output=block(data)
    assert output.shape==(2,6,4,4) and block.depthwise.groups==3
    before=block.pointwise.weight.detach().clone();assert block.training_step(data,torch.zeros_like(output),0.01)>=0.0

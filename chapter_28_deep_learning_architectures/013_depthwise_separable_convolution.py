...
if __name__ == "__main__":
    layer = DepthwiseSeparableConvolution(3, 5)
    data = torch.randn((2, 3, 8, 8))
    output = layer(data)
    assert output.shape == (2, 5, 8, 8) and layer.depthwise.groups == 3
    before = layer.pointwise.weight.detach().clone()
    assert layer.training_step(data, torch.zeros_like(output), 0.01) >= 0.0

...
def conv2d_manual(inputs: torch.Tensor, kernel: torch.Tensor, bias: torch.Tensor | None = None, stride: int = 1, padding: int = 0) -> torch.Tensor:
    """执行手写二维交叉相关卷积。
    ...
    if input_channels != kernel_channels or (bias is not None and (bias.ndim != 1 or bias.numel() != output_channels)):
    ...
    padded = torch.zeros((batch, input_channels, height + 2 * padding, width + 2 * padding), dtype=torch.float64)
    padded[:, :, padding:padding + height, padding:padding + width] = inputs.to(torch.float64)
    result = torch.zeros((batch, output_channels, output_height, output_width), dtype=torch.float64)
                    ...
                    for input_channel in range(input_channels):
                        for kernel_row in range(kernel_height):
                            for kernel_column in range(kernel_width):
                                total += float(padded[sample, input_channel, row * stride + kernel_row, column * stride + kernel_column] * weights[output_channel, input_channel, kernel_row, kernel_column])
...
if __name__ == "__main__":
    ...
    assert torch.equal(conv2d_manual(image, kernel), torch.tensor([[[[5.0]]]], dtype=torch.float64))

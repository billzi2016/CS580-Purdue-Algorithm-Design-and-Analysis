...
class RNNCell(torch.nn.Module):
    ...
    def forward(
        ...
        """
        if inputs.ndim != 3 or inputs.shape[2] != self.input_weight.shape[0]:
            raise ValueError("inputs 形状不匹配")
        batch, time, _ = inputs.shape
        state = (
            torch.zeros(
                (batch, self.bias.numel()), dtype=inputs.dtype, device=inputs.device
            )
            if initial_state is None
            else initial_state
        )
        if state.shape != (batch, self.bias.numel()):
            raise ValueError("initial_state 形状不匹配")
        states = []
        for index in range(time):
            state = torch.tanh(
                inputs[:, index, :] @ self.input_weight
                + state @ self.hidden_weight
                + self.bias
            )
            states.append(state)
        return (
            ...
            if states
                ...
                (batch, 0, self.bias.numel()), dtype=inputs.dtype, device=inputs.device
            )
        )
...
if __name__ == "__main__":
    ...
    before = cell.input_weight.detach().clone()
    assert cell.training_step(data, torch.zeros_like(output), 0.01) >= 0.0

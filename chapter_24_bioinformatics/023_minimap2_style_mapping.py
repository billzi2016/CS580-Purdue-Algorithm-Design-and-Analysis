...
def minimap2_style_map(
    reference: str, query: str, k: int, window_kmers: int, max_gap: int = 100
    ...
    anchors = [
        MappingAnchor(ref_pos, query_pos, k)
        for query_pos, word in _minimizers(query, k, window_kmers)
        for ref_pos in reference_index.get(word, [])
    ]
    if not anchors:
        return None
    ordered = sorted(anchors, key=lambda item: (item.reference_start, item.query_start))
    scores = [item.length for item in ordered]
    parent: list[int | None] = [None] * len(ordered)
    ...
    best = max(range(len(ordered)), key=lambda index: scores[index])
    path: list[MappingAnchor] = []
    while best is not None:
        path.append(ordered[best])
        best = parent[best]
    path.reverse()
    return MappingCandidate(
        tuple(path),
        max(scores),
        (path[0].reference_start, path[-1].reference_start + k),
        (path[0].query_start, path[-1].query_start + k),
    )
...
def _minimizers(sequence: str, k: int, width: int) -> list[tuple[int, str]]:
    ...
    words = [
        min(sequence[i : i + k], sequence[i : i + k].translate(COMPLEMENT)[::-1])
    ...
    for start in range(max(0, len(words) - width + 1)):
        ...
        smallest = min(window)
        position = start + max(i for i, word in enumerate(window) if word == smallest)
        if position != last:
            selected.append((position, words[position]))
            last = position
    return selected

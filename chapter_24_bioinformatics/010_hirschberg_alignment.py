...
def hirschberg_alignment(
    ...
    if len(second) > len(first):
        ...
        return HirschbergAlignment(
            reversed_result.score, reversed_result.second, reversed_result.first
        )
...
def _align(
    ...
    split = max(
        range(len(second) + 1),
        key=lambda column: forward[column] + backward[len(second) - column],
    )
    left = _align(first[:middle], second[:split], match, mismatch, gap)
    right = _align(first[middle:], second[split:], match, mismatch, gap)
    return HirschbergAlignment(
        left.score + right.score, left.first + right.first, left.second + right.second
    )
...
def _last_score_row(
    ...
    for row, symbol in enumerate(first, start=1):
        ...
        for column, other in enumerate(second, start=1):
            ...
            current.append(
                max(diagonal, previous[column] + gap, current[column - 1] + gap)
            )
        previous = current
    return previous


def _single_row_alignment(
    first: str, second: str, match: int, mismatch: int, gap: int
) -> HirschbergAlignment:
    ...
    while row or column:
        if (
            row
            and column
            and scores[row][column]
            == scores[row - 1][column - 1]
            + (match if first == second[column - 1] else mismatch)
        ):

...
def olc_assemble(reads: list[str], minimum_overlap: int = 1) -> list[str]:
    ...
    if any(any(c not in DNA for c in read) for read in reads):
        raise ValueError("reads 只能包含大写 DNA")
    contigs = list(dict.fromkeys(reads))
    contigs = [
        read
        for read in contigs
        if not any(read != other and read in other for other in contigs)
    ]
    while True:
        ...
        for i, left in enumerate(contigs):
            for j, right in enumerate(contigs):
                if i != j:
                    ...
                    if size > best_size:
                        best_size, best_left, best_right = size, i, j
        if not best_size:
            break
        merged = contigs[best_left] + contigs[best_right][best_size:]
        contigs = [
            read for k, read in enumerate(contigs) if k not in (best_left, best_right)
        ] + [merged]

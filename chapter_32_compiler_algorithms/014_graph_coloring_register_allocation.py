...
def allocate_registers(
    interference: dict[str, set[str]], register_count: int
) -> Allocation:
    """以简化—选择法为干涉图尝试 K-着色。
    ...
    while remaining:
        low_degree = next(
            (
                ...
                if len(neighbors) < register_count
            ),
            None,
        )
        node = (
            ...
            else max(remaining, key=lambda item: len(remaining[item]))
        )
        stack.append(node)
    ...
    while stack:
        ...
        used = {
            registers[neighbor]
            for neighbor in interference[node]
            if neighbor in registers
        }
        available = next(
            (color for color in range(register_count) if color not in used), None
        )
...
def _validate_graph(graph: dict[str, set[str]]) -> None:
    for node, neighbors in graph.items():
        if node in neighbors or any(
            neighbor not in graph or node not in graph[neighbor]
...
if __name__ == "__main__":
    ...
    assert all(
        allocation.registers[node] != allocation.registers[neighbor]
        for node in allocation.registers
        for neighbor in graph[node]
        if neighbor in allocation.registers
    )
